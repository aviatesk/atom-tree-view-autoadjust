...
module.exports = {
    ...
    this.subscriptions.add(
      ...
      atom.packages.onDidActivateInitialPackages(() => {
        if (atom.packages.isPackageActive('tree-view')) {
          this.init(atom.packages.getActivePackage('tree-view'));
        }
      })
    );
    // on re-activation
  ...
  init: function(treeView) {
    requestIdleCallback(() => {
      if (!treeView.mainModule.treeView) treeView.mainModule.createOrDestroyTreeViewIfNeeded()
      this.treePanel = treeView.mainModule.treeView.element;
